...
$lang = array_merge($lang, array(
	...
	'BOARD_NOT_INSTALLED_EXPLAIN'	=> '为了进行转换您必须预先安装一个全新的 phpBB3。您是否要进行【<a href="%s">全新安装</a>】？',
	...
	'COLLIDING_CLEAN_USERNAME'	=> '<strong>%s</strong> 清理自用户名:',
	...
	'CONVERT_COMPLETE_EXPLAIN'	=> '您已经将您的论坛成功转换为 phpBB 3.1，您现在可以登录并<a href="../">访问您的论坛</a>。请在开启新论坛前确认参数都已经正确设置，并删除了install安装目录。别忘了phpBB的在线使用帮助有<a href="https://www.phpbb.com/support/docs/en/3.1/ug/">文档</a>和<a href="https://www.phpbb.com/phpBB/viewforum.php?f=466">支持论坛</a>。',
	...
	'DB_HOST_EXPLAIN'			=> 'DSN 代表数据源名称，它只与 ODBC 安装有关。对于 PostgreSQL，需要通过 UNIX domain socket 用 localhost 或通过 TCP 用 127.0.0.1 连接本地服务器。对于 SQLite，需要输入数据文件的完整路径。',
	'DB_NAME'					=> '数据库名称',
	'DB_PASSWORD'				=> '数据库密码',
	'DB_PORT'					=> '数据库服务器端口',
	'DB_PORT_EXPLAIN'			=> '不用填写，除非您确定服务器监听一个非标准端口。',
	'DB_UPDATE_NOT_SUPPORTED'	=> '非常抱歉，升级程序无法升级版本低于 “%1$s” 的 phpBB 论坛。您当前使用的论坛版本为 “%2$s”。请升级至一个较新的版本后再执行此升级程序。如果您需要帮助，请到phpBB.com的用户支持版面提出。',
	'DB_USERNAME'				=> '数据库用户名',
	'DB_TEST'					=> '连接检测',
	'DEFAULT_LANG'				=> '默认论坛语言',
	...
	'INSTALL_INTRO_BODY'		=> '使用这个选项，应该可以在您的服务器上安装 phpBB.</p><p>为了继续安装，您需要知道您的数据库设置。如果您不清楚这些，请联络您的web空间服务商。没有这些信息安装将不能继续。您需要:</p>
	...
	'PHP_SETTINGS_EXPLAIN'			=> '<strong>必需的</strong> - 要安装phpBB，您必需正在运行最低 5.3.3 版本的PHP（不支持PHP 7）。如果下面出现 <var>safe mode</var>，您的PHP正在运行于安全模式，这将给远程管理及类似功能带来限制。',
	...
	'SELECT_FORUM_GA'			=> '在phpBB 3.1全局公告连接到论坛，为您当前的全局公告选择一个论坛(以后可以移动):',
	'SOFTWARE'					=> '论坛软件',
...
$lang = array_merge($lang, array(
	...
	'DOWNLOAD_UPDATE_METHOD_EXPLAIN'	=> '当您下载并解压缩档案后，您需要上载安装包内的文件到phpBB安装目录。请上传文件到各个正确位置。当您完成所有文件的上传后，请用下面的按钮做文件检查。',

	'EDIT_USERNAME'	=> '编辑用户名',
	'ERROR'			=> '错误',
	'EVERYTHING_UP_TO_DATE'		=> '您已升级到了最新phpBB版本，您现在应该<a href="%1$s">登录您的论坛</a>检查所有功能是否工作正常。别忘了删除、重命名或移动install文件夹！请给我们发送升级信息，在ACP里的<a href="%2$s">发送统计信息</a>模块可以找到有关您的服务器和论坛的配置。',
  ...
  'TRY_DOWNLOAD_METHOD_BUTTON'=> '尝试此方式',
	...
	'UPDATE_INCOMPLETE'				=> '您的 phpBB 安装并未正确的升级。',
	'UPDATE_INCOMPLETE_MORE'		=> '请阅读以下信息以修复错误。',
	'UPDATE_INCOMPLETE_EXPLAIN'		=> '<h1>Incomplete update</h1>

		<p>我们注意到您最近的升级并没有完成。访问网址 <a href="%1$s" title="%1$s">database_update script</a> 并运行它。 如果网址不存在，请 <a href="https://www.phpbb.com/downloads/" title="phpBB downloads">下载升级包</a>， 上传 "install" 文件夹到您的 phpBB 根目录（就是 config.php 所在的目录）， 然后 <a href="%1$s" title="%1$s">运行数据库升级</a>。</p>',

	'UPDATE_INSTALLATION'			=> '升级安装',
	'UPDATE_INSTALLATION_EXPLAIN'	=> '本选项将升级您的phpBB安装到最新版本。<br />在升级处理期间所有的文件都将被检查是否完整。您可以查看文件和升级前的变化。<br /><br />文件自动升级的途径有两条。</p><h2>手动升级</h2><p>这个选项，您只需要下载有改动的文件以确保不会丢失您对其他文件的修改。下载后将文件上载到phpBB目录的相应位置。之后，您可以再次进行文件检查，检查是否将文件放到了正确的位置。</p><h2>通过FTP自动升级</h2><p>这个方法和第一个类似，但是不需要下载有变动的文件和手动上传它们。系统将为您自动做这个工作。用这个方法进行升级您需要知道您的FTP登录的详细信息。 一旦完成设置，系统将重新定向到文件检查功能完成系统升级。.<br /><br />',
	'UPDATE_INSTRUCTIONS'			=> '
	...
	',
	'UPDATE_METHOD'					=> '升级方式',
	'UPDATE_METHOD_EXPLAIN'			=> '您可以选择合适的上载方式。使用FTP上载您需要提供FTP帐号的详细信息。 使用这种方法文件将自动移动到对应目录并且通过在原文件后添加.bak扩展名的方式备份原来的文件 。如果您选择下载修改的文件则您要解压缩包后手动的上载文件到相应的目录。',
